...
def prevalidate(model, val_dl, criterion, device):
    model.eval()
    loss_step = []
    for inp_data, labels in val_dl:
...
def pretrain_one_epoch(model, optimizer, train_dl, criterion,
                       device, dtype, scaler):
    model.train()
    loss_step = []
    for data in tqdm(train_dl, leave=False, desc='Training'):
...
def pretrain(model, optimizer, num_epochs, train_dl, val_dl, criterion, device):
    ...
    for epoch in pbar:
        loss_curr_epoch = pretrain_one_epoch(model, optimizer, train_dl, criterion,
                                             device=device, dtype=dtype, scaler=scaler)
        val_loss = prevalidate(model, val_dl, criterion, device)
        ...
        if val_loss < best_val_loss:
            ...
            save_model(model, './models/best_pretrained_model_min_val_loss.pth', epoch, optimizer,
                       val_loss)
    return dict_log
...
def linear_eval(model, optimizer, num_epochs, train_dl, val_dl, device):
    best_val_loss = 1e8
    best_val_acc = 0
    model = model.to(device)
    dict_log = {"train_acc_epoch": [], "val_acc_epoch": [], "loss_epoch": [], "val_loss": []}
    train_acc, _ = validate(model, train_dl, device)
    val_acc, _ = validate(model, val_dl, device)
    print(f'Init Accuracy of the model: Train:{train_acc:.3f} \t Val:{val_acc:3f}')
    pbar = tqdm(range(num_epochs))
    for epoch in pbar:
        loss_curr_epoch, train_acc = train_one_epoch(model, optimizer, train_dl, device)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                  ...
                  'optimizer_state_dict': optimizer.state_dict(),
                  'loss': val_loss,
                  }, './models/best_linear_model_min_val_loss.pth')
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                  ...
                  'optimizer_state_dict': optimizer.state_dict(),
                  'loss': val_loss,
                  }, './models/best_linear_model_max_val_acc.pth')
    return dict_log
